...
class LoginFormAuthenticator extends AbstractFormLoginAuthenticator
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, string $providerKey) # FOURTH
    {
        ...
        // Vérifier si le mot de passe correspond au mot de passe de l'administrateur
        $credentials = $this->getCredentials($request);
        if ($credentials['email'] === 'admin.admin@esprit.tn' && $credentials['password'] === 'adminadmin123_') {
            // Rediriger vers la page admin si le mot de passe correspond
            return new RedirectResponse($this->urlGenerator->generate('app_admin'));
        }

        // Rediriger vers la page d'accueil par défaut si l'utilisateur n'est pas un administrateur
        return new RedirectResponse($this->urlGenerator->generate('app_homepage'));
